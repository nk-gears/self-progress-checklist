<template>
  <div class="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-primary-700 via-purple-600 to-indigo-600 p-6">
    ...
    <div class="text-center mb-10 animate-fade-in">
      ...
      <h1 class="text-3xl font-bold text-white mb-1 tracking-tight">{{ t('login.moveTowards') }}</h1>
      <h1 class="text-4xl font-extrabold text-amber-300 mb-2 tracking-tight">{{ t('login.sampoornatha') }}</h1>
      <p class="text-primary-200 text-base">{{ t('login.tagline') }}</p>
    </div>
    ...
    <div class="w-full max-w-sm bg-white rounded-3xl p-8 shadow-2xl animate-slide-up">
      ...
      <div v-if="isMock" class="mb-6 bg-amber-50 border border-amber-200 rounded-2xl p-4 text-center">
        <p class="text-amber-700 font-semibold text-base mb-3">{{ t('login.mockMode') }}</p>
      ...
      <p v-if="loading" class="mt-4 text-center text-sm text-primary-500 font-medium animate-pulse">{{ t('login.signingIn') }}</p>
...
const handleCredential = async (response: { credential: string }) => {
  ...
  try {
    ...
    errorMsg.value = e?.message ?? t('login.signInFailed')
  } finally {
    loading.value = false
  }
}
...
onMounted(() => {
  ...
  script.onerror = () => { errorMsg.value = t('login.googleLoadFailed') }
